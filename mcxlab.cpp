...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
	...
	const char* gpuinfotag[] = {
		"name", "id", "devcount", "major", "minor", "globalmem",
	...
	for (jstruct = 0; jstruct < ncfg; jstruct++)
	{
        ...
        cfg.issaveseed=0;
#endif
			if (cfg.vol == nullptr || cfg.medianum == 0)
			{
			...
			if (nlhs >= 2)
			{
				cfg.exportdetected = (float*)malloc(
					(cfg.medianum + 1 + cfg.issaveexit * 6) * cfg.maxdetphoton * sizeof(float));
			}
			if (nlhs >= 4)
			{
...
#pragma omp parallel shared(errorflag)
{
        threadid=omp_get_thread_num();
			...
			if (nlhs >= 5)
			{
				...
				const mwSize myFieldDim[4] = { fielddim[0], fielddim[1], fielddim[2], fielddim[3] };
				mxSetFieldByNumber(plhs[2], jstruct, 0, mxCreateNumericArray(3, myFieldDim, mxUINT8_CLASS, mxREAL));
				//
				if (cfg.debuglevel & MCX_DEBUG_MOVE)
					memcpy((float*)mxGetPr(mxGetFieldByNumber(plhs[4], jstruct, 0)), cfg.exportdebugdata,
					       fielddim[0] * fielddim[1] * sizeof(float));
				if (cfg.exportdebugdata)
					free(cfg.exportdebugdata);
				cfg.exportdebugdata = nullptr;
			}
			if (nlhs >= 4)
			{
				...
				mxSetFieldByNumber(plhs[2], jstruct, 0, mxCreateNumericArray(3, myFieldDim, mxUINT8_CLASS, mxREAL));
				//
				memcpy((unsigned char*)mxGetPr(mxGetFieldByNumber(plhs[3], jstruct, 0)), cfg.seeddata,
				       fielddim[0] * fielddim[1]);
				free(cfg.seeddata);
				cfg.seeddata = nullptr;
			}
			if (nlhs >= 3)
			{
			...
			if (nlhs >= 2)
			{
				...
				{
					const mwSize myFieldDim[4] = { fielddim[0], fielddim[1], fielddim[2], fielddim[3] };
					mxSetFieldByNumber(plhs[1], jstruct, 0, mxCreateNumericArray(2, myFieldDim, mxSINGLE_CLASS, mxREAL));
					//
					memcpy((float*)mxGetPr(mxGetFieldByNumber(plhs[1], jstruct, 0)), cfg.exportdetected,
					       fielddim[0] * fielddim[1] * sizeof(float));
				}
				free(cfg.exportdetected);
				cfg.exportdetected = nullptr;
			}
			if (nlhs >= 1)
			{
				int fieldlen;
				...
				if (cfg.issaveref)
				{
					...
					const mwSize myFieldDim[4] = { fielddim[0], fielddim[1], fielddim[2], fielddim[3] };
					mxSetFieldByNumber(plhs[0], jstruct, 2, mxCreateNumericArray(4, myFieldDim, mxSINGLE_CLASS, mxREAL));
					//
					memcpy((float*)mxGetPr(mxGetFieldByNumber(plhs[0], jstruct, 2)), dref, fieldlen * sizeof(float));
					free(dref);
				}
				const mwSize myFieldDim[4] = { fielddim[0], fielddim[1], fielddim[2], fielddim[3] };
				mxSetFieldByNumber(plhs[0], jstruct, 0, mxCreateNumericArray(4, myFieldDim, mxSINGLE_CLASS, mxREAL));

				memcpy((float*)mxGetPr(mxGetFieldByNumber(plhs[0], jstruct, 0)), cfg.exportfield,
				       fieldlen * sizeof(float));
				free(cfg.exportfield);
				cfg.exportfield = nullptr;
...
void mcx_set_field(const mxArray* root, const mxArray* item, int idx, Config* cfg)
{
	...
	char* jsonshapes = nullptr;
	int i, j;

	if (strcmp(name, "nphoton") == 0 && cfg->replay.seed != nullptr)
		return;
	...
	else if (strcmp(name, "srctype") == 0)
	{
		...
		const char* srctypeid[] = {
			"pencil", "isotropic", "cone", "gaussian", "planar", "pattern", "fourier", "arcsine", "disk", "fourierx",
			"fourierx2d", "zgaussian", "line", "slit", "pencilarray","donut","pattern3d", "weighed","weighed3d","fiber","fibergauss","cyllens",""
		};
	...
	else if (strcmp(name, "srcpattern") == 0)
	{
		...
		//okay, srcpattern is 60x60x560 in this case, what variable is representative of this 
...
void mcx_replay_prep(Config* cfg)
{
	...
	for (i = 0; i < dimdetps[1]; i++)
		...
		{
			if (i != cfg->nphoton)
				memcpy((char *)(cfg->replay.seed) + cfg->nphoton * seedbyte, (char *)(cfg->replay.seed) + i * seedbyte,
				       seedbyte);
...
void mcx_validate_config(Config* cfg)
{
	...
	if ((cfg->srctype == MCX_SRC_PATTERN || cfg->srctype == MCX_SRC_PATTERN3D || cfg->srctype == MCX_SRC_WEIGHED) && cfg->srcpattern == nullptr)
	...
	if (cfg->issavedet == 0)
		cfg->issaveexit = 0;
	if (cfg->seed < 0 && cfg->seed != SEED_FROM_FILE) cfg->seed = time(nullptr);
