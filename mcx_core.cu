...
in an void voxel, mcx advances the photon in v.{xyz} direction
until it hits an non-zero voxel */
__device__ inline int findClosest(float arr[], int n, float target)
{
	// Corner cases 
	if (target <= arr[0])
		return 0;
...
__device__ inline int launchnewphoton(MCXpos *p,MCXdir *v,MCXtime *f,float3* rv,Medium *prop,uint *idx1d,
	  ...
	  *((float4*)f)=float4(0.f,0.f,gcfg->minaccumtime,f->ndone);
	  ...
	  switch(mcxsource) {
		case(MCX_SRC_WEIGHED):{
		      float rx=rand_uniform01(t);
 //put it outside of this block so that it doesnt suck up ernergy generating randos
				  //make rx be >0 and <1
				  // this code works very efficiently  so we'll keep for now 
				  int index = findClosest(srcpattern,gcfg->dimlen.z,rx);
				 
				  //////what do we do with index now? reconstruct 3d index from it 
				  //// some concern about using the modulo operator here but well keep it for now 
				  //// trying ths code n a reg c++ project seems to work 
			  	  int max_x = gcfg->dimlen.x;
				  int max_y = gcfg->dimlen.y/gcfg->dimlen.x;
				  int x_local = index % max_x;
				  int y_local = (index / max_x) % max_y;
				  int z_local = index / (max_x * max_y);
				  *((float4*)p) = float4(p->x + x_local,
					  p->y + y_local,
					  p->z + z_local,
					  p->w);
				  //set weight to what srcpattern says to 
				  // instead, turn weight to 1, so that we sample positiions acording to excitation but the launch is the same
				  //p->w = srcpattern[index];
				  p->w = 1;
				  //in this case choose a random point within diostribution set by excitation and laucnhed isotropically 
				  ...
				  *Lmove = 0.f;*idx1d=(int(floorf(p->z))*gcfg->dimlen.y+int(floorf(p->y))*gcfg->dimlen.x+int(floorf(p->x)));
		...
		case(MCX_SRC_WEIGHED3D):
	  	{ 
			  ...
			  if (gcfg->srctype == MCX_SRC_PATTERN3D) {
				  //wasting photons right now, we are equally distrbuting to low and high photons 
				//what we really want is to get index from inverse sampling of weights from pattern 
			  ...
			  else if(gcfg->srctype == MCX_SRC_WEIGHED3D)
			  {
		...
		case(MCX_SRC_FIBERGAUSS): {
			...
			sincosf(ang, &stheta, &ctheta);
			rotatevector(v, stheta, ctheta, sphi, cphi);
			//this is how to evenly distribute r
			r = sqrtf(rand_uniform01(t)) * gcfg->srcparam1.y ;
			p->x += r * cphi;
			p->y += r * sphi;
			*idx1d = (int(floorf(p->z)) * gcfg->dimlen.y + int(floorf(p->y)) * gcfg->dimlen.x + int(floorf(p->x)));
			if (p->x < 0.f || p->y < 0.f || p->z < 0.f || p->x >= gcfg->maxidx.x || p->y >= gcfg->maxidx.y || p->z >=
				gcfg->maxidx.z)
			{
				*mediaid = 0;
			}
		...
		case(MCX_SRC_FIBER):
	  	{
			...
			else if (gcfg->srctype == MCX_SRC_DONUT)
			{
				r = sqrtf(rand_uniform01(t)) * gcfg->srcparam1.x + gcfg->srcparam1.y;
			}
			else if(gcfg->srctype == MCX_SRC_FIBER)
			{
				//do it all in here 
				//step 1, do a gaussian distribution R, and the 2pi has been done upstairs 
				r = sqrtf(-0.5f * logf(rand_uniform01(t))) * gcfg->srcparam1.x;
				////already moved like a gaussian disk, so now we need to move its direction vector according to an angle
				float ang, stheta_local, ctheta_local, sphi_local, cphi_local;
				ang = TWO_PI * rand_uniform01(t); //next arimuth angle
				sincosf(ang, &sphi_local, &cphi_local);
				do
				{
					ang = acosf(2.f * rand_uniform01(t) - 1.f); //sine distribution
				}
				while (ang > gcfg->srcparam1.y);
				sincosf(ang, &stheta_local, &ctheta_local);
				rotatevector(v, stheta_local, ctheta_local, sphi_local, cphi_local);
				*Lmove = 0.f;
			}
		...
		case(MCX_SRC_LINE):
		case(MCX_SRC_SLIT):
		case(MCX_SRC_CYLLENS):
	  	{
			  // this is ok for now because we never intend to use slit of line and ours will always be on the surface 
			  float rx = rand_uniform01(t);
				//pass in rx as variable for r just to keep everything nice and compilable without having to delete a bunch of stuff
				//speify the center point and the xy dim left or right of the center point so that we can easily keep track in our coordinate system
			  float r = rx;
			  float ry = rand_uniform01(t);
			  *((float4*)p) = float4(p->x+(rx-0.5f)*gcfg->srcparam1.x,
				  p->y+(ry-0.5f)*gcfg->srcparam1.y,
				  p->z,
				  p->w);
			  ...
			  if (gcfg->srctype == MCX_SRC_CYLLENS)
			  {
					// same as slit but we just need to ask some sideways divergence according to the focal length
					// http://mathworld.wolfram.com/SpherePointPicking.html
					//intiate variables for zenith and arithumal direction, but do not bother getting arithumal angle since that is set to 2pi pr 0 only vares in one direction 
					float ang, stheta, ctheta, sphi, cphi;
					//ang = TWO_PI * rand_uniform01(t); //next arimuth angle
					ang = 0;
					// and now figure out new angle within the conical surface in one direction
					sincosf(ang, &sphi, &cphi);
					// a solid-angle section of a gaussian sphere, do gaussian to make faster
					ang = sqrtf(-2.f*logf(rand_uniform01(t)))*(1.f - 2.f*rand_uniform01(t))*gcfg->srcparam1.z;
					sincosf(ang, &stheta, &ctheta);
					rotatevector(v, stheta, ctheta, sphi, cphi);
					*Lmove = 0.f;
					break;
			  }
				//rv means reciprocal velocity, is ths what it should be defined as? 
...
#pragma omp barrier
     ...
     *progress = 0;
	 ...
	 else if (cfg->srctype == MCX_SRC_PATTERN3D || cfg->srctype == MCX_SRC_WEIGHED)
	 {
		 // allocate space on the gpu to hold the srcpattern from user
...
#pragma omp barrier
     ...
     if(cfg->srcpattern)
     {
		 ...
		 else if(cfg->srctype == MCX_SRC_WEIGHED)
		 {
			 //https://stackoverflow.com/questions/2386772/what-is-the-difference-between-float-and-double
			 // a float does not have the precison we need , so we have to somehoow truncate the signal
			 //Get sum of srcpattern
			 float sum = 0;
			 for (int i = 0; i < dimlen.z; i++)
			 {
				 if(cfg->srcpattern[i]<1e-5)
				 {
					 sum += 0;
				 }else
				 {
					 sum += cfg->srcpattern[i];
				 }
			 }
			 // get cumsum of pattern 
			 for(int i = 1;i<dimlen.z;i++)
			 {
				 cfg->srcpattern[i] = cfg->srcpattern[i] + cfg->srcpattern[i - 1];
			 }
			 //divide cmsum by sum 
			 for (int i = 0; i < dimlen.z; i++)
			 {
				 cfg->srcpattern[i] /= sum;
				 if(isnan(cfg->srcpattern[i]))
				 {
					 cfg->srcpattern[i] = 0;
				 }
			 }
			 // pass this as srcpattern into gpu
			 CUDA_ASSERT(cudaMemcpy(gsrcpattern, cfg->srcpattern, sizeof(float)*(int)(cfg->srcparam1.x*cfg->srcparam1.y*cfg->srcparam1.z), cudaMemcpyHostToDevice));
		 }
     }
...
#pragma omp master
{
	   ...
	   switch(cfg->srctype) {
		...
		case(MCX_SRC_DONUT): mcx_main_loop<MCX_SRC_DONUT> << <mcgrid, mcblock, sharedbuf >> > (gmedia, gfield, genergy, gPseed, gPpos, gPdir, gPlen, gPdet, gdetected, gsrcpattern, greplayw, greplaytof, gseeddata, gdebugdata, gprogress); break;
		...
		case(MCX_SRC_WEIGHED): mcx_main_loop<MCX_SRC_WEIGHED> << <mcgrid, mcblock, sharedbuf >> > (gmedia, gfield, genergy, gPseed, gPpos, gPdir, gPlen, gPdet, gdetected, gsrcpattern, greplayw, greplaytof, gseeddata, gdebugdata, gprogress); break;
		case(MCX_SRC_FIBER): mcx_main_loop<MCX_SRC_FIBER> << <mcgrid, mcblock, sharedbuf >> > (gmedia, gfield, genergy, gPseed, gPpos, gPdir, gPlen, gPdet, gdetected, gsrcpattern, greplayw, greplaytof, gseeddata, gdebugdata, gprogress); break;
		case(MCX_SRC_FIBERGAUSS): mcx_main_loop<MCX_SRC_FIBERGAUSS> << <mcgrid, mcblock, sharedbuf >> > (gmedia, gfield, genergy, gPseed, gPpos, gPdir, gPlen, gPdet, gdetected, gsrcpattern, greplayw, greplaytof, gseeddata, gdebugdata, gprogress); break;
		case(MCX_SRC_CYLLENS): mcx_main_loop<MCX_SRC_CYLLENS> << <mcgrid, mcblock, sharedbuf >> > (gmedia, gfield, genergy, gPseed, gPpos, gPdir, gPlen, gPdet, gdetected, gsrcpattern, greplayw, greplaytof, gseeddata, gdebugdata, gprogress); break;
