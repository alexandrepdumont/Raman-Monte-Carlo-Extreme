...
int (*Rasterizers[])(cJSON* obj, Grid3D* g) = {
	NULL, mcx_raster_origin, mcx_raster_grid, mcx_raster_subgrid,
...
int mcx_load_jsonshapes(Grid3D* g, char* fname)
{
	...
	if (strstr(fname, ".json") != NULL)
	{
		...
		jbuf[len - 1] = '\0';
		fclose(fp);
		if ((err = mcx_parse_shapestring(g, jbuf)))
		{
			/*error msg is generated inside*/
			free(jbuf);
			return err;
		}
		free(jbuf);
			...
			for (k = 0; k < g->dim->z; k++)
				for (j = 0; j < g->dim->y; j++)
					for (i = bd[p]; i < bd[p + 1]; i++)
						(*(g->vol))[g->rowmajor ? i * dimyz + j * g->dim->z + k : k * dimxy + j * g->dim->x + i] = bd[p
							+ 2];
	}
	else if (dir == 1)
	{
